<?php
	...
	try
  {
    ...
    $cinemasEtFilms = $helper->showtimesByPosition($lat, $long, $radius=7);
		...
		// Commencer la chaîne des markers
		$markers = '[';
		...
		foreach ($cinemasEtFilms->theaterShowtimes as $cinemaEtFilms)
		{
			...
			// Ajouter le cinéma courant aux markers
			$markers = $markers.'{
				"title":\''.$nomCinema.'\',
		...
		// Terminer la chaîne des markers
		$markers = $markers.']';

		// Afficher le javascript du HTML
						...
						<script type="text/javascript">
							...
							function LoadMap() {
								...
								for (var i = 0; i < markers.length; i++) {
									...
									document.getElementById("label-map").innerHTML = "Emplacement des cinemas a proximite :<br><br>";
								}
							}
						</script>');
	}
